def euler11():
    ...
    for i in range(20):
        for j in range(20):
            # right
            if j+3 < 20:
                if (m[i][j]*m[i][j+1]*m[i][j+2]*m[i][j+3] > max_product):
                    max_product = m[i][j]*m[i][j+1]*m[i][j+2]*m[i][j+3]

            # down
            if i+3 < 20:
                if (m[i][j]*m[i+1][j]*m[i+2][j]*m[i+3][j] > max_product):
                    max_product = m[i][j]*m[i+1][j]*m[i+2][j]*m[i+3][j]

            # right-down cross
            if j+3 < 20 and i+3 < 20:
                if (m[i][j]*m[i+1][j+1]*m[i+2][j+2]*m[i+3][j+3] > max_product):
                    max_product = m[i][j]*m[i+1][j+1]*m[i+2][j+2]*m[i+3][j+3]
